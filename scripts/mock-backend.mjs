...
// It serves agent cards at /rest/v1/registry-center/agent-cards.
...
const registryServer = http.createServer((req, res) => {
    ...
    if (path === '/rest/v1/registry-center/agent-cards' && req.method === 'GET') {
        res.writeHead(200, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ agentCards: REGISTRY_AGENT_CARDS }));
        return;
    }
...
registryServer.listen(5000, process.env.HOST || '127.0.0.1', () => {
...
server.listen(5001, process.env.HOST || '127.0.0.1', () => {
