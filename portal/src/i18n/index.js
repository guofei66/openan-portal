...
export function registerPluginI18n(namespace, resources) {
    ...
    // CRITICAL: i18next only resolves "ns:key" lookups for namespaces listed in
    // options.ns. Registering the bundle alone is not enough — a Header
    // t('execution-center:execution.title') call would return the raw key unless
    // the ns is declared. Append it (idempotent).
    if (namespace && !i18n.options.ns.includes(namespace)) {
